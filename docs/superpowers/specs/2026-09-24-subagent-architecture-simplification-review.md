...
| Tool registry / tools | Native registry plus profile-level tool selection; the subagent builder must not add hidden tools | Exact tool execution is a sound invariant. The `SAFE_DEFAULT` mode currently selects all registered read-only tools, which may be a broad prompt and authority surface despite its name. | Measure the actual effective tool count/schema-token cost. Consider a deliberately small default set or an explicit no-tools default in a future product decision; do not silently change current behavior. Custom/MCP tools are not clearly part of subagent preflight today; declare them unsupported or design their registration/permission resolution explicitly. |
...
- Measure SAFE_DEFAULT tool count, schema token impact, and actual subagent use.
- Trace the child `ask` path for argument-sensitive policy from tool call through task terminal status.
- Confirm whether restart notification delivery is a product requirement for every deployment mode.
...
- Remove `invoke` only after caller audit and a deprecation window.
- Retire `LEGACY` only after migration/telemetry shows no remaining legacy profiles.
- Reconsider SAFE_DEFAULT breadth only after usage/token measurements and product approval.
...
1. Does the product require durable completion notification after restart in all modes, or only durable `subagent_check` status?
2. Should SAFE_DEFAULT mean all currently safe read-only tools, a small curated set, or no tools until explicitly selected?
3. What is the supported runtime outcome when an argument-specific governance check returns `ask` inside a child?
4. Are custom tools and MCP tools intentionally unsupported for subagents, or should they participate in the same preflight contract?
5. What is the task/outbox retention policy, and can an outbox record safely reference a task row indefinitely?
6. Which legacy profile versions must remain supported, and for how long?
