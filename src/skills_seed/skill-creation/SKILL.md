...
4. **Launch the viewer** with both qualitative outputs and quantitative data:
   ...
   **Headless environments:** If a browser is not available, use `--static <output_path>` to write a standalone HTML file instead of starting a server. Feedback will be downloaded as a `feedback.json` file when the user clicks "Submit All Reviews". After download, copy `feedback.json` into the workspace directory for the next iteration to pick up.
...
- **No browser display** — save viewer HTML as static file or render via `canvas-painting`
- **Skills directory** — shown in your system prompt
- **Files available** — `files_write`, `files_read`, `files_edit`, `files_delete`, `skills_load`, `skills_reload`, `subagent_start`
- **Canvas for review** — use `canvas-painting` (loaded via `skills_load`) to render comparison cards

### Simplified Evaluation

When you can't launch the eval viewer:
1. Save test outputs to the workspace directory
2. Grade inline (no grader subagent needed)
3. Aggregate to `benchmark.json` manually
4. Load `canvas-painting` and output a comparison card with `html:canvas` fence block
5. User reviews on Canvas, provides feedback
