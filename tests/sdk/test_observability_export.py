...
- export operational spans only — Langfuse instrumentation-scope spans are
  dropped (their prompt/completion content belongs to the admin's Langfuse,
  never to the operational telemetry);
...
def test_operational_telemetry_span_exported_with_trace_id_and_allowed_attrs():
...
def test_delegate_failure_is_swallowed_and_exporter_keeps_working():
    ...
    delegate = RecordingExporter()
    exporter = FilteringSpanExporter(delegate)
    span = _make_span("assistant.operational", {"http.request.method": "GET"})
...
def test_mixed_batch_drops_langfuse_keeps_operational():
    from src.sdk.observability import FilteringSpanExporter

    delegate = RecordingExporter()
    exporter = FilteringSpanExporter(delegate)
    operational = _make_span("assistant.operational", {"http.route": "/health"})
    lf_gen = _make_span("langfuse", {"prompt.text": "SECRET"})

    exporter.export([lf_gen, operational])
...
def _make_span_with_exception():
    ...
    provider = SDKTracerProvider()
    tracer = provider.get_tracer("assistant.operational")
...
def test_exception_event_message_and_status_description_dropped():
    ...
    for event in exported.events:
        assert set(event.attributes or {}) <= set(ALLOWED_OPERATIONAL_ATTRIBUTES)
        assert not any(
...
def test_exporter_built_only_with_explicit_endpoint(obs_reset, monkeypatch):
    ...
    assert obs._state["owned_operational_processors"] == []
    ...
    assert len(obs._state["owned_operational_processors"]) == 1
